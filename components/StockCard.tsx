...
export const StockCard: React.FC<StockCardProps> = ({
  ...
  const score = typeof tech?.score === 'number' ? tech.score : 0;
  ...
  // Signals (guard for non-array)
  const signals = Array.isArray(tech?.activeSignals)
    ? tech!.activeSignals.slice(0, 3)
    : [];

  // RSI / MACD / EMA values & classes (null-safe)
  const rsi = tech?.rsi;
  ...
  const macdClass =
    macd && typeof macd.histogram === 'number'
      ? macd.histogram > 0
        ? 'text-green-400'
        : 'text-red-400'
      : 'text-slate-500';
  ...
  const macdValue =
    macd && typeof macd.macd === 'number'
      ? macd.macd.toFixed(1)
      : '--';

  const ema9Value =
    ema && typeof ema.ema9 === 'number'
      ? ema.ema9.toFixed(0)
      : '--';
    ...
    <div className="bg-surface rounded-xl p-4 border border-slate-700 hover:border-blue-500 transition-all duration-200 shadow-lg group relative overflow-hidden">
      ...
      <div className="absolute top-0 right-0 p-2 text-right">
        <div className="bg-slate-900/80 backdrop-blur-sm border border-slate-700 rounded-lg px-2 py-1 text-xs font-bold font-mono flex items-center justify-end gap-1 mb-1">
          ...
          Score: {tech ? score.toFixed(0) : '--'}
        </div>
        <div
          className={`text-[10px] font-bold uppercase tracking-wider ${strengthColor}`}
        >
          {strength}
        </div>
      </div>
      ...
      <div className="flex justify-between items-start mb-3 pr-20">
        <div>
          <h3 className="text-lg font-bold text-white flex items-center gap-2">
            ...
            {stock.timeframe && (
              <span
                className={`text-[9px] px-1.5 py-0.5 rounded border font-bold uppercase ${getTimeframeColor(
                  stock.timeframe
                )}`}
              >
                {stock.timeframe}
      ...
      <div className="flex justify-between items-end mb-4">
        ...
        <div className="text-right">
          <div className="text-[10px] text-slate-400 flex items-center justify-end gap-1 mb-0.5">
      ...
      <div className="flex items-center gap-2 mb-3 bg-indigo-500/10 border border-indigo-500/20 p-2 rounded-lg">
        ...
        <div className="text-xs">
          ...
          <span className="text-white italic">
            {stock.chartPattern || 'Trend Follow'}
          </span>
        </div>
      </div>
      ...
      <div className="mb-4 space-y-2">
        ...
        <div className="grid grid-cols-3 gap-1 bg-slate-900/50 p-2 rounded-lg text-[10px] font-mono mt-2">
          ...
          <div>
            ...
            <span className={macdClass}>{macdValue}</span>
          </div>
          <div>
            <span className="text-slate-500 block">EMA(9)</span>
            <span className="text-slate-300">{ema9Value}</span>
          </div>
        </div>
      </div>
      ...
      <button
        ...
        aria-label={`Execute trade for ${stock.symbol}`}
      >
        Execute Trade
      </button>
    </div>
