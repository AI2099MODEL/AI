    ...
    <div className="p-4 pb-20 animate-fade-in space-y-6">
       <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-[10px] md:text-xs text-slate-400 font-medium">{subtitle}</p>
          </div>
       </div>

        {/* Live Summary Card */}
       <div className="bg-gradient-to-br from-blue-900/40 via-slate-900 to-black rounded-2xl border border-blue-500/20 p-5 md:p-6 shadow-2xl relative overflow-hidden group">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-4 relative z-10">
                <div>
                    <p className="text-blue-300 text-[10px] md:text-xs font-bold uppercase tracking-wider mb-1 flex items-center gap-1"><TrendingUp size={12}/> Total P&L</p>
                ...
                <div className="text-left sm:text-right">
                    <p className="text-blue-300 text-[10px] md:text-xs font-bold uppercase tracking-wider mb-1">Total Invested</p>
                    <div className="text-lg md:text-2xl font-mono font-bold text-white break-words">
                        ₹{totalCost.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                    </div>
                    <p className="text-[10px] md:text-xs text-slate-400 mt-2 font-medium">Value: <span className="text-white">₹{currentVal.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span></p>
                </div>
            </div>
            ...
            <div className="bg-black/30 backdrop-blur-md p-3 rounded-xl border border-white/10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                 <div className="flex items-center gap-2 text-blue-200 text-[10px] md:text-xs font-bold"><Wallet size={14} className="text-blue-400"/> Total Broker Cash</div>
                 <div className="font-mono font-bold text-white text-base md:text-lg break-all">₹{totalCash.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</div>
            </div>
       </div>
        ...
        <div className="grid grid-cols-2 gap-3">
            {Object.entries(brokerBalances).map(([broker, balance]) => (
                <div key={broker} className="p-3 bg-surface rounded-xl border border-slate-700 shadow-lg relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-2 mb-2 text-[10px] md:text-xs font-bold text-slate-300 uppercase tracking-wide">
                        <Building2 size={12} className="text-blue-500"/> {broker}
                    </div>
                    <div className="text-xs md:text-sm font-mono text-white font-bold break-words">₹{Number(balance).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</div>
                </div>
            ))}
        </div>
       )}
